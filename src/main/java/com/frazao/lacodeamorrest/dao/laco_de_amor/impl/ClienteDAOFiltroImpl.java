...
import com.frazao.lacodeamorrest.dao.laco_de_amor.ClienteDAOFiltro;
...
public class ClienteDAOFiltroImpl implements ClienteDAOFiltro {
	...
	@Override
	public Collection<Cliente> filtrar(final ClienteFiltroDTO f) {

		final StringBuilder sql = new StringBuilder();
		sql.append("SELECT em.*").append("\n");
		sql.append("FROM   ").append(this.databaseSchema).append(".cliente as em").append("\n");
		...
		final Query query = this.entityManager.createNativeQuery(sql.toString(), Cliente.class);
		if (ObjectUtils.isNotEmpty(idSim)) {
