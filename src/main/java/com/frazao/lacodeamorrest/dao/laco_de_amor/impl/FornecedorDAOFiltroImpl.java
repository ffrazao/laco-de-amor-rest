...
public class FornecedorDAOFiltroImpl implements FornecedorDAOFiltro {
	...
	public Collection<Fornecedor> filtrar(final FornecedorFiltroDTO f) {
		...
		sql.append("FROM   ").append(this.databaseSchema).append(".fornecedor as em").append("\n");
		...
		final Query query = this.entityManager.createNativeQuery(sql.toString(), Fornecedor.class);
		if (ObjectUtils.isNotEmpty(idSim)) {
